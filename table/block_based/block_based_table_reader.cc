...
Status BlockBasedTable::Clone(
  ...
  auto rep = std::unique_ptr<BlockBasedTable::Rep>(new BlockBasedTable::Rep(
      ...
      true /*skip_filters*/, rep_->file_size, rep_->level,
      rep_->immortal_table));
  rep->status = rep_->status;
  rep->file = rep_->file;
  rep->base_cache_key = rep_->base_cache_key;
  rep->persistent_cache_options = rep_->persistent_cache_options;
  rep->footer = rep_->footer;
  rep->filter_type = rep_->filter_type;
  rep->filter_handle = rep_->filter_handle;
  rep->compression_dict_handle = rep_->compression_dict_handle;
  rep->table_properties = rep_->table_properties;
  rep->index_type = rep_->index_type;
  ...
  if (rep_->internal_prefix_transform != nullptr) {
    ...
    if (transform_impl == nullptr || transform_impl->user_prefix_extractor() !=
      ...
      s = Status::InvalidArgument("Unexpected internal_prefix_transform");
      return s;
    }
  }
  ...
  s = new_table->CreateIndexReader(
      ro, prefetch_buffer.get(), nullptr /*meta_iter*/, use_cache, prefetch,
      pin, &lookup_context, &new_table->rep_->index_reader);
  if (!s.ok()) {
    return s;
  }
  if (new_table->rep_->filter_policy) {
    new_table->rep_->filter = new_table->CreateFilterBlockReader(
        ro, prefetch_buffer.get(), use_cache, prefetch, pin, &lookup_context);
  }
  if (!new_table->rep_->compression_dict_handle.IsNull()) {
    std::unique_ptr<UncompressionDictReader> uncompression_dict_reader;
    s = UncompressionDictReader::Create(
        new_table.get(), ro, prefetch_buffer.get(), use_cache, prefetch, pin,
        &lookup_context, &new_table->rep_->uncompression_dict_reader);
    if (!s.ok()) {
      return s;
    }
  }
