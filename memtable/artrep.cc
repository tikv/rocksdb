...
public:
 ...
 void Get(const LookupKey& k, void* callback_args,
   ...
   AdaptiveRadixTreeRep::Iterator iter(&skip_list_);
   Slice dummy_slice;
  ...
  class Iterator : public MemTableRep::Iterator {
    typename SkipList<const MemTableRep::KeyComparator&>::Iterator iter_;
  ...
  class LookaheadIterator : public MemTableRep::Iterator {
   public:
    explicit LookaheadIterator(const AdaptiveRadixTreeRep<SkipList>& rep) :
   ...
   private:
    const AdaptiveRadixTreeRep<SkipList>& rep_;
    typename SkipList<const MemTableRep::KeyComparator&>::Iterator iter_;
    typename SkipList<const MemTableRep::KeyComparator&>::Iterator prev_;
  };

  MemTableRep::Iterator* GetIterator(Arena* arena = nullptr) override {
    if (lookahead_ > 0) {
      void *mem =
        arena ? arena->AllocateAligned(sizeof(AdaptiveRadixTreeRep::LookaheadIterator))
              : operator new(sizeof(AdaptiveRadixTreeRep::LookaheadIterator));
      return new (mem) AdaptiveRadixTreeRep::LookaheadIterator(*this);
    } else {
      void *mem =
...
MemTableRep* SkipListFactory::CreateMemTableRep(
  ...
  return new AdaptiveRadixTreeRep<InlineSkipList>(compare, allocator, transform, lookahead_);
}

MemTableRep* DoublySkipListFactory::CreateMemTableRep(
    const MemTableRep::KeyComparator& compare, Allocator* allocator,
    const SliceTransform* transform, Logger* /*logger*/) {
  return new AdaptiveRadixTreeRep<DoublySkipList>(compare, allocator, transform, lookahead_);
}
