...
namespace rocksdb {
namespace {
class DoublyLinkedSkipListRep : public MemTableRep {
  DoublyLinkedSkipList<const MemTableRep::KeyComparator&> skip_list_;
...
public:
 explicit DoublyLinkedSkipListRep(const MemTableRep::KeyComparator& compare,
 ...
 void Get(const LookupKey& k, void* callback_args,
   ...
   DoublyLinkedSkipListRep::Iterator iter(&skip_list_);
   Slice dummy_slice;
  ...
  ~DoublyLinkedSkipListRep() override {}

  // Iteration over the contents of a skip list
  class Iterator : public MemTableRep::Iterator {
    DoublyLinkedSkipList<const MemTableRep::KeyComparator&>::Iterator iter_;

   public:
    // Initialize an iterator over the specified list.
    // The returned iterator is not valid.
    explicit Iterator(
        const DoublyLinkedSkipList<const MemTableRep::KeyComparator&>* list)
        : iter_(list) {}
  ...
  class LookaheadIterator : public MemTableRep::Iterator {
   public:
    explicit LookaheadIterator(const DoublyLinkedSkipListRep& rep) :
   ...
   private:
    const DoublyLinkedSkipListRep& rep_;
    DoublyLinkedSkipList<const MemTableRep::KeyComparator&>::Iterator iter_;
    DoublyLinkedSkipList<const MemTableRep::KeyComparator&>::Iterator prev_;
  };

  MemTableRep::Iterator* GetIterator(Arena* arena = nullptr) override {
    if (lookahead_ > 0) {
      void *mem =
        arena ? arena->AllocateAligned(sizeof(DoublyLinkedSkipListRep::LookaheadIterator))
              : operator new(sizeof(DoublyLinkedSkipListRep::LookaheadIterator));
      return new (mem) DoublyLinkedSkipListRep::LookaheadIterator(*this);
    } else {
      void *mem =
        arena ? arena->AllocateAligned(sizeof(DoublyLinkedSkipListRep::Iterator))
              : operator new(sizeof(DoublyLinkedSkipListRep::Iterator));
      return new (mem) DoublyLinkedSkipListRep::Iterator(&skip_list_);
    }
  }
};
}

class DoublyLinkedSkipListFactory : public MemTableRepFactory {
 public:
  explicit DoublyLinkedSkipListFactory(size_t lookahead = 0) : lookahead_(lookahead) {}

  using MemTableRepFactory::CreateMemTableRep;
  virtual MemTableRep* CreateMemTableRep(const MemTableRep::KeyComparator& compare,
                                         Allocator* allocator, const SliceTransform* transform,
                                         Logger* logger) override {
      return new DoublyLinkedSkipListRep(compare, allocator, transform, lookahead_);
  }
...
MemTableRepFactory* NewDoublyLinkedSkipListRepFactory(size_t lookahead) {
    return new DoublyLinkedSkipListFactory(lookahead);
}
