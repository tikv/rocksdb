...
#include "memtable/doubly_linked_skiplist.h"
#include <set>
...
typedef DoublyLinkedSkipList<TestComparator> TestDoublyLinkedSkipList;

class DoublyLinkedSkipTest : public testing::Test {
 public:
  void Insert(TestDoublyLinkedSkipList* list, Key key) {
  ...
  bool InsertWithHint(TestDoublyLinkedSkipList* list, Key key, void** hint) {
    char* buf = list->AllocateKey(sizeof(Key));
    memcpy(buf, &key, sizeof(Key));
  ...
  void Validate(TestDoublyLinkedSkipList* list) {
    // Check keys exist.
    for (Key key : keys_) {
    ...
    // keys exist.
    TestDoublyLinkedSkipList::Iterator iter(list);
...
TEST_F(DoublyLinkedSkipTest, Empty) {
  Arena arena;
  TestComparator cmp;
  DoublyLinkedSkipList<TestComparator> list(cmp, &arena);
  Key key = 10;
  ASSERT_TRUE(!list.Contains(Encode(&key)));

  DoublyLinkedSkipList<TestComparator>::Iterator iter(&list);
  ASSERT_TRUE(!iter.Valid());
  iter.SeekToFirst();
  ASSERT_TRUE(!iter.Valid());
  key = 100;
...
TEST_F(DoublyLinkedSkipTest, InsertAndLookup) {
  const int N = 2000;
  const int R = 5000;
  Random rnd(1000);
  std::set<Key> keys;
...
TEST_F(DoublyLinkedSkipTest, InsertWithHint_Sequential) {
  const int N = 100000;
  Arena arena;
  TestComparator cmp;
  TestDoublyLinkedSkipList list(cmp, &arena);
  void* hint = nullptr;
  for (int i = 0; i < N; i++) {
    Key key = i;
...
TEST_F(DoublyLinkedSkipTest, InsertWithHint_MultipleHints) {
  const int N = 100000;
  const int S = 100;
  Random rnd(534);
  Arena arena;
...
TEST_F(DoublyLinkedSkipTest, InsertWithHint_MultipleHintsRandom) {
  const int N = 100000;
  const int S = 100;
  Random rnd(534);
  Arena arena;
...
TEST_F(DoublyLinkedSkipTest, InsertWithHint_CompatibleWithInsertWithoutHint) {
  const int N = 100000;
  const int S1 = 100;
  const int S2 = 100;
  Random rnd(534);
...
class ConcurrentTest {
 ...
 private:
  ...
  // DoublyLinkedSkipList is not protected by mu_.  We just use a single writer
  // thread to modify it.
  DoublyLinkedSkipList<TestComparator> list_;
 ...
 public:
  ...
  void ReadStep(Random* rnd) {
    ...
    DoublyLinkedSkipList<TestComparator>::Iterator iter(&list_);
...
// scaffolding.
TEST_F(DoublyLinkedSkipTest, ConcurrentReadWithoutThreads) {
  ConcurrentTest test;
...
TEST_F(DoublyLinkedSkipTest, ConcurrentInsertWithoutThreads) {
  ConcurrentTest test;
  Random rnd(test::RandomSeed());
  for (int i = 0; i < 10000; i++) {
    test.ReadStep(&rnd);
...
TEST_F(DoublyLinkedSkipTest, ConcurrentRead1) { RunConcurrentRead(1); }
TEST_F(DoublyLinkedSkipTest, ConcurrentRead2) { RunConcurrentRead(2); }
TEST_F(DoublyLinkedSkipTest, ConcurrentRead3) { RunConcurrentRead(3); }
TEST_F(DoublyLinkedSkipTest, ConcurrentRead4) { RunConcurrentRead(4); }
TEST_F(DoublyLinkedSkipTest, ConcurrentRead5) { RunConcurrentRead(5); }
TEST_F(DoublyLinkedSkipTest, ConcurrentInsert1) { RunConcurrentInsert(1); }
TEST_F(DoublyLinkedSkipTest, ConcurrentInsert2) { RunConcurrentInsert(2); }
TEST_F(DoublyLinkedSkipTest, ConcurrentInsert3) { RunConcurrentInsert(3); }
