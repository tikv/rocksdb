...
// DoublySkipList will not be destroyed while the read is in progress.
// Apart from that, reads progress without any internal locking or
// synchronization.
//
// Invariants:
//
// (1) Allocated nodes are never deleted until the DoublySkipList is
// destroyed.  This is trivially guaranteed by the code since we never
// delete any skip list nodes.
//
// (2) The contents of a Node except for the next/prev pointers are
// immutable after the Node has been linked into the DoublySkipList.
...
class DoublySkipList {
 ...
 public:
  ...
  // Create a new DoublySkipList object that will use "cmp" for comparing
  ...
  // skiplist object.
  explicit DoublySkipList(Comparator cmp, Allocator* allocator,
  ...
  class Iterator {
   public:
    ...
    explicit Iterator(const DoublySkipList* list);

    // Change the underlying skiplist used for this iterator
    // This enables us not changing the iterator without deallocating
    // an old one and then allocating a new one
    void SetList(const DoublySkipList* list);
 ...
 private:
  ...
  // No copying allowed
  DoublySkipList(const DoublySkipList&);
  DoublySkipList& operator=(const DoublySkipList&);
};

// Implementation details follow

template <class Comparator>
struct DoublySkipList<Comparator>::Splice {
...
template <class Comparator>
struct DoublySkipList<Comparator>::Node {
...
template <class Comparator>
inline DoublySkipList<Comparator>::Iterator::Iterator(
    const DoublySkipList* list) {
  SetList(list);
}

template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::SetList(
    const DoublySkipList* list) {
  list_ = list;
  node_ = nullptr;
}
...
template <class Comparator>
inline bool DoublySkipList<Comparator>::Iterator::Valid() const {
  return node_ != nullptr;
}

template <class Comparator>
inline const char* DoublySkipList<Comparator>::Iterator::key() const {
  assert(Valid());
  return node_->Key();
}

template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::Next() {
  assert(Valid());
  node_ = node_->Next(0);
}

template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::Prev() {
...
template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::Seek(const char* target) {
  node_ = list_->FindGreaterOrEqual(target);
}

template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::SeekForPrev(
    const char* target) {
  Seek(target);
  if (!Valid()) {
    SeekToLast();
...
template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::SeekToFirst() {
  node_ = list_->head_->Next(0);
}

template <class Comparator>
inline void DoublySkipList<Comparator>::Iterator::SeekToLast() {
...
template <class Comparator>
int DoublySkipList<Comparator>::RandomHeight() {
...
template <class Comparator>
bool DoublySkipList<Comparator>::KeyIsAfterNode(const char* key,
                                                Node* n) const {
...
template <class Comparator>
bool DoublySkipList<Comparator>::KeyIsAfterNode(const DecodedKey& key,
                                                Node* n) const {
  // nullptr n is considered infinite
  assert(n != head_);
  return (n != nullptr) && (compare_(n->Key(), key) < 0);
...
DoublySkipList<Comparator>::FindGreaterOrEqual(const char* key) const {
...
template <class Comparator>
typename DoublySkipList<Comparator>::Node*
DoublySkipList<Comparator>::FindLessThan(const char* key, Node** prev) const {
  return FindLessThan(key, prev, head_, GetMaxHeight(), 0);
}

template <class Comparator>
typename DoublySkipList<Comparator>::Node*
DoublySkipList<Comparator>::FindLessThan(const char* key, Node** prev,
...
template <class Comparator>
typename DoublySkipList<Comparator>::Node*
DoublySkipList<Comparator>::FindLast() const {
...
template <class Comparator>
DoublySkipList<Comparator>::DoublySkipList(const Comparator cmp,
                                           Allocator* allocator,
                                           int32_t max_height,
...
template <class Comparator>
char* DoublySkipList<Comparator>::AllocateKey(size_t key_size) {
  return const_cast<char*>(AllocateNode(key_size, RandomHeight())->Key());
}

template <class Comparator>
typename DoublySkipList<Comparator>::Node*
DoublySkipList<Comparator>::AllocateNode(size_t key_size, int height) {
...
DoublySkipList<Comparator>::AllocateSplice() {
...
template <class Comparator>
bool DoublySkipList<Comparator>::InsertConcurrently(const char* key) {
...
template <class Comparator>
bool DoublySkipList<Comparator>::InsertWithHint(const char* key, void** hint) {
  assert(hint != nullptr);
...
template <class Comparator>
void DoublySkipList<Comparator>::RecomputeSpliceLevels(const DecodedKey& key,
                                                       Splice* splice,
                                                       int recompute_level) {
...
template <class Comparator>
template <bool UseCAS>
bool DoublySkipList<Comparator>::Insert(const char* key, Splice* splice,
...
template <class Comparator>
bool DoublySkipList<Comparator>::Contains(const char* key) const {
...
template <class Comparator>
void DoublySkipList<Comparator>::TEST_Validate() const {
