...
Status DBImpl::MultiBatchWriteImpl(const WriteOptions& write_options,
  ...
  if (writer.state == WriteThread::STATE_PARALLEL_MEMTABLE_WRITER) {
    ...
    // `running`, the value of `running` is always larger than one if the leader
    // thread does not call `CompleteParallelMemTableWriter`.
    ...
    // WriteGroup, and it would not notify the threads in this WriteGroup. So we
    // must make someone in this WriteGroup to complete it and leader thread is
    // easy to be decided.
    if (is_leader_thread) {
