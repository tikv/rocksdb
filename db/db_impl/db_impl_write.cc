...
async_result DBImpl::AsyncWriteImpl(const WriteOptions& write_options,
  ...
  assert(!two_write_queues_ || !disable_memtable);
  {
    ...
    LogContext log_context(need_log_sync);
    status = PreprocessWrite(write_options, &log_context, &write_context);
    if (!two_write_queues_) {
      // Assign it after ::PreprocessWrite since the sequence might advance
      // inside it by WriteRecoverableState
  ...
  if (status.ok()) {
    ...
    size_t pre_release_callback_cnt = 0;
    for (auto* writer : write_group) {
      auto &batch{writer->multi_batch.batches[0]};
      if (writer->CheckCallback(this)) {
        valid_batches += writer->batch_cnt;
        if (writer->ShouldWriteToMemtable()) {
          total_count += WriteBatchInternal::Count(batch);
          parallel = parallel && !batch->HasMerge();
        }
        total_byte_size = WriteBatchInternal::AppendedByteSize(
            total_byte_size, WriteBatchInternal::ByteSize(batch));
        if (writer->pre_release_callback) {
          pre_release_callback_cnt++;
        }
      }
    ...
    if (!two_write_queues_) {
      if (status.ok() && !write_options.disableWAL) {
        PERF_TIMER_GUARD(write_wal_time);
        auto result =
            AsyncWriteToWAL(write_group, log_writer, log_used,
  ...
  if (need_log_sync) {
    ...
    if (status.ok()) {
      MarkLogsSynced(logfile_number_, need_log_dir_sync, &synced_wals);
    } else {
      MarkLogsNotSynced(logfile_number_);
    }
    ...
    if (w.status.ok()) {
      ...
      for (auto writer : wal_write_group) {
        if (writer->CheckCallback(this)) {
          ...
          total_byte_size = WriteBatchInternal::AppendedByteSize(
              total_byte_size, WriteBatchInternal::ByteSize(writer->multi_batch.batches[0]));
        }
      }
      if (w.disable_wal) {
        has_unpersisted_data_.store(true, std::memory_order_relaxed);
