...
// Pending request
struct GenericRateLimiterV2::Req {
  explicit Req(int64_t _bytes, port::Mutex* _mu)
      : request_bytes(_bytes), bytes(_bytes), cv(_mu), granted(false) {}
  int64_t request_bytes;
  int64_t bytes;
...
GenericRateLimiterV2::GenericRateLimiterV2(int64_t rate_bytes_per_sec,
                                           int64_t refill_period_us,
                                           int32_t fairness,
                                           RateLimiter::Mode mode, Env* env,
                                           bool auto_tuned)
    : RateLimiter(mode),
      refill_period_us_(refill_period_us),
      rate_bytes_per_sec_(auto_tuned ? rate_bytes_per_sec / 2
...
GenericRateLimiterV2::~GenericRateLimiterV2() {
  MutexLock g(&request_mutex_);
  stop_ = true;
  requests_to_wait_ = static_cast<int32_t>(queue_[Env::IO_LOW].size() +
                                           queue_[Env::IO_HIGH].size());
...
// This API allows user to dynamically change rate limiter's bytes per second.
void GenericRateLimiterV2::SetBytesPerSecond(int64_t bytes_per_second) {
  assert(bytes_per_second > 0);
  rate_bytes_per_sec_ = bytes_per_second;
  refill_bytes_per_period_.store(
      CalculateRefillBytesPerPeriod(bytes_per_second),
...
void GenericRateLimiterV2::Request(int64_t bytes, const Env::IOPriority pri,
                                   Statistics* stats) {
  TEST_SYNC_POINT("GenericRateLimiterV2::Request");
  TEST_SYNC_POINT_CALLBACK("GenericRateLimiterV2::Request:1",
                           &rate_bytes_per_sec_);
  if (pri == Env::IO_HIGH) {
...
void GenericRateLimiterV2::Refill() {
  TEST_SYNC_POINT("GenericRateLimiterV2::Refill");
  next_refill_us_ = NowMicrosMonotonic(env_) + refill_period_us_;
  // Carry over the left over quota from the last period
  auto refill_bytes_per_period =
      refill_bytes_per_period_.load(std::memory_order_relaxed);
...
int64_t GenericRateLimiterV2::CalculateRefillBytesPerPeriod(
    int64_t rate_bytes_per_sec) {
  if (port::kMaxInt64 / rate_bytes_per_sec < refill_period_us_) {
    // Avoid unexpected result in the overflow case. The result now is still
    // inaccurate but is a number that is large enough.
...
Status GenericRateLimiterV2::Tune() {
  ...
  int64_t new_bytes_per_sec =
      ...
      std::max(highpri_bytes_sampler_.GetRecentValue() / 10, kHighBytesLower);
...
RateLimiter* NewGenericRateLimiterV2(
    int64_t rate_bytes_per_sec, int64_t refill_period_us /* = 100 * 1000 */,
    int32_t fairness /* = 10 */,
    RateLimiter::Mode mode /* = RateLimiter::Mode::kWritesOnly */,
    bool auto_tuned /* = false */) {
  ...
  assert(refill_period_us > 0);
  assert(fairness > 0);
  return new GenericRateLimiterV2(rate_bytes_per_sec, refill_period_us,
                                  fairness, mode, Env::Default(), auto_tuned);
}
